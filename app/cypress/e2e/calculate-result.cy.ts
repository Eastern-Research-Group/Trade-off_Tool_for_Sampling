describe('Calculate results tests', () => {
  ...
  it('Verify empty samples', () => {
    ...
    cy.findByRole('button', { name: 'View Detailed Results' })
      .should('exist')
      .click();
    cy.findByText(
      'There are no samples to run calculations on. Please add samples and try again.',
    ).should('exist');
  });
  ...
  it('Calculate wet-vac samples', () => {
    ...
    const summarys = [
      ...
      'Time to Prepare Kits (person hours):',
      'Time to Collect (person hours):',
      'Sampling Material Cost ($):',
      'Sampling Personnel Labor Cost ($):',
      'Analysis',
      'Time to Analyze (person hours):',
      'Analysis Labor Cost ($):',
  ...
  it('Verify traning-mode View Contamination Hits without file', () => {
    ...
    cy.findByRole('button', { name: 'Calculate Resources' })
      .should('exist')
      .click({ force: true });
